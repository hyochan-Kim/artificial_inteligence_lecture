...
def first_floor(maze,start,key,end,m,n):
    ...
    while heap:
        ...
        for i,j in can_go:
            ...
            new_distance = heuristic(neighbor,key)
            ...
            if neighbor in close_set and new_distance >= fdistance.get(neighbor,0):
                continue

            if new_distance < fdistance.get(neighbor,0) or neighbor not in [i[1] for i in heap]:
                fdistance[neighbor] = new_distance
    ...
    while heap2:
        ...
        for i, j in can_go:
            ...
            if 0 <= neighbor[0] < m:
                ...
                # array bound x walls
                continue
            if neighbor in close_set2 and new_distance >= fdistance2.get(neighbor,0):
                continue

            if new_distance < fdistance.get(neighbor,0) or neighbor not in [i[1]for i in heap2]:
                came_from2[neighbor] = now
                fdistance2[neighbor] = heuristic(neighbor, key)
